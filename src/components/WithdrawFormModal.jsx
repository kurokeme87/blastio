...
const WithdrawFormModal = ({ setOpenWithdrawModal }) => {
    const tokens = [
        {
            value: "ETH",
            name: "Eth",
                    ...
                    <div className="rounded-[6px] p-[1px] bg-camo-400 pointer-events-auto" aria-modal="true" aria-label="Select token" role="dialog" style={{ clipPath: 'polygon(101.5px 24px, 125.5px 0px, 100% 0px, 100% calc(100% - 56px), calc(100% - 56px) 100%, 0px 100%, 0px 60px, 36px 24px)' }}>
                        <div className="relative h-max rounded-[5px] bg-camo-700 w-max px-6 xs:px-8 pb-14 pt-16" style={{ clipPath: 'polygon(101.5px 24px, 125.5px 0px, 100% 0px, 100% calc(100% - 56px), calc(100% - 56px) 100%, 0px 100%, 0px 60px, 36px 24px)' }}>
                            ...
                            <div className="divide-y divide-camo-400">
                                {tokens.map((token, i) => {
                                    return (
                                        <button key={i} disabled value={token.value} className="group flex w-72 items-center gap-2 p-2 transition-colors enabled:hover:bg-camo-600 enabled:hover:text-white disabled:text-camo-300 text-yellow-100">
                                            <img alt="token" loading="lazy" width="24" height="24" decoding="async" data-nimg="1" src={token.imgSrc} style={{ color: 'transparent' }} />
                                            <div className="typography-brand-heading-3 text-camo-300 text-current">{token.symbol}</div>
